...
export default function LoginPage() {
  ...
  const getLoginMessage = (error: string) => {
    ...
    if (normalized.includes("user not found")) return t.auth.userNotFound;
    ...
    <div className="min-h-screen dark:bg-[#050816] bg-[#F8FAFC] pt-28 pb-16 px-4">
      <div className="max-w-md mx-auto rounded-3xl dark:bg-[#0d1117]/80 bg-white border dark:border-white/5 border-slate-200/80 shadow-xl p-7">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-2xl font-black dark:text-white text-slate-900">{t.auth.loginTitle}</h1>
        </div>
        <div className="space-y-4">
          ...
          <button onClick={submit} disabled={loading || !email || !password} className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-blue-600 to-teal-500 text-white font-bold disabled:opacity-50">
            {t.auth.loginButton}
          </button>
        ...
        <div className="mt-5 flex items-center justify-between text-sm">
          <Link href="/forgot-password" className="text-blue-500 font-semibold">{t.auth.forgotLink}</Link>
          <Link href="/register" className="dark:text-slate-300 text-slate-600">{t.auth.noAccount}</Link>
        </div>
      </div>
    </div>
  );
